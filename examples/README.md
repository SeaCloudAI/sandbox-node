...
4. `build-template.mjs`: template/build workflows through `client.build` plus `templateBuild()`
...
Recommended path: the example uses the root `SandboxClient`, `client.build`, and `templateBuild()`.
